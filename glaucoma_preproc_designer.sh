...
bids_dir=/misc/tanner/danielacoutino/glaucoma/bids
  ...
  if [ $isOK -eq 0 ]; then echolor red "[ERROR] Cannot preprocess hyperband."
    ...
    if [ ! -f $fcheck ]; then
        ...
        mrconvert \
          ...
          my_do_cmd mrconvert \
           -export_grad_fsl $bids_dir/derivatives/sub-${sID}/dwi/sub-${sID}_acq-hb_dwi_de_designer.{bvec,bval} \
           -json_export $bids_dir/derivatives/sub-${sID}/dwi/sub-${sID}_acq-hb_dwi_de_designer.json \
          $bids_dir/derivatives/sub-${sID}/dwi/sub-${sID}_acq-hb_dwi_de_designer.mif \
          $bids_dir/derivatives/sub-${sID}/dwi/sub-${sID}_acq-hb_dwi_de_designer.nii.gz
    ...
    if [ ! -f $fcheck ]; then
     singularity run --nv -B $bids_dir $designer_container designer \
        ...
        mrconvert \
          ...
          my_do_cmd mrconvert \
           -export_grad_fsl $bids_dir/derivatives/sub-${sID}/dwi/sub-${sID}_acq-muse_dwi_de_designer.{bvec,bval} \
           -json_export $bids_dir/derivatives/sub-${sID}/dwi/sub-${sID}_acq-muse_dwi_de_designer.json \
          $bids_dir/derivatives/sub-${sID}/dwi/sub-${sID}_acq-muse_dwi_de_designer.mif \
          $bids_dir/derivatives/sub-${sID}/dwi/sub-${sID}_acq-muse_dwi_de_designer.nii.gz
    else 
